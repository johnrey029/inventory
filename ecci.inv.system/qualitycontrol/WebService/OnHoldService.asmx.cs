...
namespace ecci.inv.system.qualitycontrol.WebService
{
    ...
    public class OnHoldService : System.Web.Services.WebService
    {
        ...
        public void GetOnHoldItems()
        {
            ...
            WHERE s.quantity>0 and r.postatus != 'Returned' and s.status != 'Evaluated'
            ORDER BY s.id ASC");
            while (con._dr.Read())
            {
        ...
        public int UpdateDispatch(int upid, int rework, int returns, int scrap)
        {
                ...
                if (oq == sum)
                {
                    ...
                    if (returns > 0)
                    {
                        ...
                        {
                            con.OpenConection();
                            con.ExecSqlQuery("INSERT INTO stock_raw(purchaseorder, itemsid, quantity, receivedquantity, dispatchquantity, purchasedate, deliverydate, postatus, unit,price)VALUES(@po, @item, @quan, @rq,@dq, @pdate, @ddate, @stat,  @unit, @price)");
                            ...
                            con.Cmd.Parameters.Add("@price", SqlDbType.Money).Value = 0.00;
